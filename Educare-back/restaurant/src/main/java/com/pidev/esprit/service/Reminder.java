...
public class Reminder {
    ...
    public LocalDateTime getNextMealTime(LocalTime currentTime) {
        LocalDateTime now = LocalDateTime.now();
        LocalDate currentDate = now.toLocalDate();
        // Set the next meal time to breakfast time on the current date
        LocalDateTime nextMealDateTime = LocalDateTime.of(currentDate, BREAKFAST_TIME);

        if (currentTime.isAfter(DINNER_TIME)) { // if breakfast is tomorrow
    ...
    public String getTimeDescription(LocalTime time) {
